...
def notebook_exec(**kwargs):
  ...
  parameters = dict(
    test_size_ = 0.25, 
    dataset = dags_path+"data/ecomerce.csv", 
    model = dags_path+"model/", 
    n_estimators = 1000, 
    max_depth = 5
  )
)
...
with DAG(dag_id="pipeline_data_science_papermill_python",
    ...
    run_notebook_task = PythonOperator(
      ...
      op_kwargs={   
        'input_notebook': dags_path+'notebooks/regression-project.ipynb',
        'output': dags_path+'notebooks/output-{{ execution_date }}.ipynb'
      }
    )
